...
- Name: pdf-download.spec.ts >> PDF 다운로드 기능 >> 클라이언트명에 특수문자가 있을 때 파일명이 올바르게 생성되어야 함
- Location: __tests__/e2e/pdf-download.spec.ts:41:7
...
Call log:
  - waiting for locator('text=/클라이언트/').first()
