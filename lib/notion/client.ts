...
export const INVOICE_DB_ID = toCanonicalId(process.env.NOTION_DATABASE_ID ?? '');
export const ITEMS_DB_ID = toCanonicalId(process.env.NOTION_ITEMS_DATABASE_ID ?? '');

export function validateEnv(): void {
  if (!NOTION_API_KEY) throw new Error('NOTION_API_KEY 환경변수가 설정되지 않았습니다.');
  if (!INVOICE_DB_ID) throw new Error('NOTION_DATABASE_ID 환경변수가 설정되지 않았습니다.');
  if (!ITEMS_DB_ID) throw new Error('NOTION_ITEMS_DATABASE_ID 환경변수가 설정되지 않았습니다.');
}
