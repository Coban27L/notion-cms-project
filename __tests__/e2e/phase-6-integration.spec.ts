...
test.describe('Phase 6: 관리자 대시보드 통합 테스트', () => {
  test('관리자 로그인 → 대시보드 진입 → UI 노출 검증 (데스크톱)', async ({ page }) => {
  ...
  test('뷰 전환 (카드/테이블)', async ({ page }) => {
    ...
    // 카드 확인
    const quoteCard = page.locator('[class*="rounded-xl"][class*="border"]').first();
  ...
  test('로그아웃 플로우', async ({ page }) => {
    ...
    // 사이드바 로그아웃 버튼
    const sidebarLogout = page.locator('aside button:has-text("로그아웃")').first();
  ...
  test('복합 필터 (검색 + 상태 + 정렬)', async ({ page }) => {
    ...
    // URL 파라미터 확인
    await page.waitForURL(/search=테스트.*status=발행.*sort=amount-desc|status=발행.*search=테스트.*sort=amount-desc/);
