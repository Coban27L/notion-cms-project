...
test.describe('접근성 감시 (a11y Audit)', () => {
  ...
  test('공유 링크 페이지 - 견적서 상세 접근성', async ({ page }) => {
    ...
    // 상호작용 요소 확인 (PDF 다운로드)
    const downloadButton = page.locator('button:has-text("PDF 다운로드")').first();
  ...
  test('폼 접근성 - 라벨과 입력 필드 연결', async ({ page }) => {
    ...
    // 이메일 입력 필드
    const emailInput = page.locator('input[type="email"]');
    const emailLabel = page.locator('label').filter({ has: emailInput }).first();
  ...
  test('키보드 네비게이션 - Tab 순서', async ({ page }) => {
    ...
    // Tab 키 이동
    await page.keyboard.press('Tab');
    let focusedAfterTab = await page.evaluate(() => {
      const el = document.activeElement as HTMLElement;
      return el?.tagName.toLowerCase();
    });

    // 상호작용 요소로 포커스 이동해야 함
    expect(['a', 'button', 'input', 'select', 'textarea'].includes(focusedAfterTab)).toBeTruthy();
    ...
    expect(['a', 'button', 'input', 'select', 'textarea'].includes(focusedAfterSecondTab)).toBeTruthy();
  });
