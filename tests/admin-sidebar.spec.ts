...
test.describe('AdminSidebar 네비게이션', () => {
  ...
  test('모바일에서 햄버거 메뉴 클릭 시 드로어 열림', async ({ page }) => {
    ...
    const hamburgerButton = page.locator('button:has(svg[data-lucide="menu"])').first();
  ...
  test('관리자 정보 카드 표시', async ({ page }) => {
    ...
    await expect(email).toContainText(process.env.ADMIN_EMAIL || 'admin@example.com');
  });
  ...
  test('사이드바 접기/펼치기 토글', async ({ page }) => {
    ...
    // 초기 상태: 사이드바 펼쳐짐
    const sidebar = page.locator('aside');
    let width = await sidebar.evaluate((el: HTMLElement) => (el as HTMLElement).offsetWidth);
    ...
    width = await sidebar.evaluate((el: HTMLElement) => (el as HTMLElement).offsetWidth);
  ...
  test('로그아웃 버튼 클릭 시 세션 종료 및 로그인 페이지 리디렉션', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
  ...
  test('로컬스토리지 상태 저장 (접기/펼치기)', async ({ page }) => {
    ...
    // 상태 유지 확인
    const sidebar = page.locator('aside');
    const width = await sidebar.evaluate((el: HTMLElement) => (el as HTMLElement).offsetWidth);
